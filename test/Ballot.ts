...
describe("Ballot", async () => {
  describe("when the contract is deployed", async () => {
    it("has the provided proposals", async () => {
      ...
      expect(proposal0).to.eq(PROPOSALS[0]);
      const proposal1 = await ballotContract.read.proposals([1n]);
      expect(proposal1).to.eq(PROPOSALS[0]);
      const proposal2 = await ballotContract.read.proposals([2n]);
      expect(proposal2).to.eq(PROPOSALS[0]);
